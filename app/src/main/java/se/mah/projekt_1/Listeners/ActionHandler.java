package se.mah.projekt_1.Listeners;
...
/**
 * Created by Gustaf Bohlin on 15/04/2016.
 */
public class ActionHandler extends ActionBarDrawerToggle implements View.OnClickListener, DatePickerDialog.OnDateSetListener, ViewPager.OnPageChangeListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public void onDateSet(DatePicker datePicker, int i, int i1, int i2) {
        ...
        if(datePicker.equals(dPFrom)) {
            calendarFrom.set(i, i1, i2, 0, 0, 1);   //Get the start of the day instead of right now
            dateFrom = new CalendarFormatter(calendarFrom);
            controller.onDateFromSet(dateFrom);
        }
        else if(datePicker.equals(dPTo)) {
            calendarTo.set(i, i1, i2, 23, 59, 59);  //Get the end of the day instead of right now
